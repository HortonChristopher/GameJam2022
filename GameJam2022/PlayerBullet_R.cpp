...
ID3D12Device* PlayerBullet_R::device = nullptr;
ID3D12GraphicsCommandList* PlayerBullet_R::cmdList = nullptr;
PlayerBullet_R::PipelineSet PlayerBullet_R::pipelineSet;
Camera* PlayerBullet_R::camera = nullptr;
XMVECTOR PlayerBullet_R::velocity_;

void PlayerBullet_R::StaticInitialize(ID3D12Device* device, Camera* camera)
{
	// nullptrチェック nullptr check
	assert(device);

	PlayerBullet_R::device = device;
	PlayerBullet_R::camera = camera;
...
void PlayerBullet_R::CreateGraphicsPipeline()
{
...
void PlayerBullet_R::PreDraw(ID3D12GraphicsCommandList* cmdList)
{
	// PreDrawとPostDrawがペアで呼ばれていなければエラー Error if PreDraw and PostDraw are not called in pairs
	assert(PlayerBullet_R::cmdList == nullptr);

	// コマンドリストをセット Set command list
	PlayerBullet_R::cmdList = cmdList;

	// プリミティブ形状を設定 Set primitive shape
	cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}

void PlayerBullet_R::PostDraw()
{
	// コマンドリストを解除 Cancel command list
	PlayerBullet_R::cmdList = nullptr;
}

std::unique_ptr<PlayerBullet_R> PlayerBullet_R::Create(Model* model, Camera* camera, XMFLOAT3 pos, XMFLOAT3 rot, const XMVECTOR& velocity)
{
	// 3Dオブジェクトのインスタンスを生成 Instantiate a 3D object
	PlayerBullet_R* playerBullet = new PlayerBullet_R();
	...
	return std::unique_ptr<PlayerBullet_R>(playerBullet);
}

PlayerBullet_R::~PlayerBullet_R()
{
	if (collider) {
...
bool PlayerBullet_R::Initialize(XMFLOAT3 pos, XMFLOAT3 rot)
{
	// nullptrチェック nullptr check
	assert(device);
...
void PlayerBullet_R::Draw()
{
	// nullptrチェック nullptr check
	assert(device);
	assert(PlayerBullet_R::cmdList);
...
void PlayerBullet_R::SetCollider(BaseCollider* collider)
{
...
void PlayerBullet_R::UpdateWorldMatrix()
{
	assert(camera);
