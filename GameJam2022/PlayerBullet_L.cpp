...
ID3D12Device* PlayerBullet_L::device = nullptr;
ID3D12GraphicsCommandList* PlayerBullet_L::cmdList = nullptr;
PlayerBullet_L::PipelineSet PlayerBullet_L::pipelineSet;
Camera* PlayerBullet_L::camera = nullptr;
XMVECTOR PlayerBullet_L::velocity_;

void PlayerBullet_L::StaticInitialize(ID3D12Device* device, Camera* camera)
{
	// nullptrチェック nullptr check
	assert(device);

	PlayerBullet_L::device = device;
	PlayerBullet_L::camera = camera;
...
void PlayerBullet_L::CreateGraphicsPipeline()
{
...
void PlayerBullet_L::PreDraw(ID3D12GraphicsCommandList* cmdList)
{
	// PreDrawとPostDrawがペアで呼ばれていなければエラー Error if PreDraw and PostDraw are not called in pairs
	assert(PlayerBullet_L::cmdList == nullptr);

	// コマンドリストをセット Set command list
	PlayerBullet_L::cmdList = cmdList;

	// プリミティブ形状を設定 Set primitive shape
	cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}

void PlayerBullet_L::PostDraw()
{
	// コマンドリストを解除 Cancel command list
	PlayerBullet_L::cmdList = nullptr;
}

std::unique_ptr<PlayerBullet_L> PlayerBullet_L::Create(Model* model, Camera* camera, XMFLOAT3 pos, XMFLOAT3 rot, const XMVECTOR& velocity)
{
	// 3Dオブジェクトのインスタンスを生成 Instantiate a 3D object
	PlayerBullet_L* playerBullet = new PlayerBullet_L();
	...
	return std::unique_ptr<PlayerBullet_L>(playerBullet);
}

PlayerBullet_L::~PlayerBullet_L()
{
	if (collider) {
...
bool PlayerBullet_L::Initialize(XMFLOAT3 pos, XMFLOAT3 rot)
{
	// nullptrチェック nullptr check
	assert(device);
...
void PlayerBullet_L::Draw()
{
	// nullptrチェック nullptr check
	assert(device);
	assert(PlayerBullet_L::cmdList);
...
void PlayerBullet_L::SetCollider(BaseCollider* collider)
{
...
void PlayerBullet_L::UpdateWorldMatrix()
{
	assert(camera);
